...
(function(){
  ...
  saveApiKey(){ const input=document.getElementById('api-key-input'); const key=input.value.trim(); if (key){ if(!this.validateApiKey(key)){ this.showStatus('❌ Invalid API key format','error'); return;} this.apiKey=key; this.keyExpiry=Date.now()+this.maxKeyAge; this.preferredModelUrl=null; this.discoveredModelUrls=null; this.saveApiKeyToFile(key); input.value=''; this.updateAIStatus(true); this.primeModels(); this.showStatus(`🔒 API key saved to config file!\nAuto-expires in ${this.maxKeyAge/60000} minutes per session`,'success'); setTimeout(()=>this.hideSettings(),2000);} else { this.showStatus('Please enter a valid API key','error'); } }
    ...
    bindEvents(){ const toggle=document.getElementById('assistant-toggle'); const close=document.getElementById('assistant-close'); const settings=document.getElementById('assistant-settings'); const settingsClose=document.getElementById('settings-close'); const saveKey=document.getElementById('save-api-key'); const testAI=document.getElementById('test-ai'); const sendBtn=document.getElementById('send-message'); const input=document.getElementById('chat-input'); const voiceBtn=document.getElementById('voice-input'); const quickBtns=document.querySelectorAll('.quick-btn'); const suggestionBtns=document.querySelectorAll('.suggestion-btn'); const exportChat=document.getElementById('export-chat'); const clearChat=document.getElementById('clear-chat');
      ...
      if (testAI) testAI.addEventListener('click',()=>this.testAI());
      if (sendBtn) sendBtn.addEventListener('click',()=>this.sendMessage());
      if (voiceBtn) voiceBtn.addEventListener('click',()=>this.toggleVoiceInput());
      if (exportChat) exportChat.addEventListener('click',()=>this.exportChatHistory());
      if (clearChat) clearChat.addEventListener('click',()=>this.clearChatHistory());
      if (input) {
        input.addEventListener('keydown',(e)=>{ if(e.key==='Enter'&&!e.shiftKey){ e.preventDefault(); this.sendMessage(); } });
        input.addEventListener('input',()=>{ this.updateCharCounter(); this.autoResize(); });
        input.addEventListener('focus',()=>{ const status=document.getElementById('input-status'); if(status) status.classList.remove('hidden'); });
        input.addEventListener('blur',()=>{ const status=document.getElementById('input-status'); if(status) status.classList.add('hidden'); });
      }
      quickBtns.forEach(btn=>{ btn.addEventListener('click',()=>{ const question=btn.getAttribute('data-question'); this.handleQuickQuestion(question); }); });
      suggestionBtns.forEach(btn=>{ btn.addEventListener('click',()=>{ const text=btn.getAttribute('data-text'); if (input){ input.value=text; this.updateCharCounter(); this.autoResize(); input.focus(); } }); }); }
    showSettings(){ const panel=document.getElementById('settings-panel'); if(!panel) return; panel.classList.remove('hidden'); const msgs=document.getElementById('chat-messages'); if(msgs) msgs.style.display='none'; const qa=document.getElementById('quick-actions'); if(qa) qa.style.display='none'; }
    hideSettings(){ const panel=document.getElementById('settings-panel'); if(!panel) return; panel.classList.add('hidden'); const msgs=document.getElementById('chat-messages'); if(msgs) msgs.style.display='block'; const qa=document.getElementById('quick-actions'); if(qa) qa.style.display='block'; }
